...
describe('readBufferBody', () => {
  it('reads buffer body from a request', async () => {
    ...
    const bufferBodySelector = async () => {
    ...
    await testHandler(handler, async url => {
      ...
      expect(response.headers['content-length']).toBe(targetBuffer.length.toString())
    })
  })
  ...
  it('throws 413 error if the request body is bigger than limits', async () => {
    ...
    await testHandler(handler, async url => {
      const targetBuffer = Buffer.from('Peter Piper picked a peck of pickled peppers')
...
describe('readJsonBody', () => {
  it('reads and parse JSON from a request body', async () => {
    ...
    await testHandler(handler, async url => {
      ...
      expect(response.headers['content-length']).toBe(JSON.stringify(target).length.toString())
    })
  })
